...
/*********************************************************
  ...
  Remarks:
    None                                                  
  *********************************************************/
#define MEDIA_SOFT_DETECT 1 //jbai
...
MMC_RESPONSE SendMMCCmd(BYTE cmd, DWORD address)
{
    ...
    timeout = NCR_TIMEOUT; //jbai ncr_timeout = 14
    do
    {
        response.r1._byte = MDD_SDSPI_ReadMedia();
        //response.r1._byte = MDD_SDSPI_ReadMedia(); //jbai
        timeout--;
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    do
    {
		...
		response = SendMMCCmd(SEND_CSD, 0x00); //jbai FF, 40, C8 for byte1
        timeout--;
    }while((response.r1._byte != 0x00) && (timeout != 0));
    if(timeout != 0x00)
    {
